...
# Configuração da IA (Chave fornecida)
genai.configure(api_key="AIzaSyCJ72jm7JfJKINgV9SjEALYdTwEGlM3FMU")
model = genai.GenerativeModel('gemini-1.5-flash')

def obter_sugestao_ia(setor, regiao, qtd):
    prompt = f"Como consultor de SST, sugira 3 melhorias ergonômicas práticas para o setor {setor} com {qtd} queixas na região {regiao}. Seja breve e técnico."
    try:
        return model.generate_content(prompt).text
    except:
        return "Sugestões dinâmicas temporariamente indisponíveis."

# Estilização para Celular e Cards
st.markdown("""
    <style>
...
# --- PROCESSAMENTO SEGURO (Baseado na sua planilha real) ---
df_base = df_raw.copy()
...
mes_sel = st.sidebar.selectbox("Período:", ["Todos os Meses"] + meses)
...
# KPIs que se ajustam sozinhos ao celular
k1, k2 = st.columns(2)
total = len(df_f)
...
if not df_sim.empty:
    ...
    # Remove eventuais nulos de forma segura
    ...
    # IA com Expander (Melhor no celular)
    with st.expander("🤖 Ver Análise Técnica da IA", expanded=True):
        st.write(obter_sugestao_ia(setor_sel, reg_topo, qtd_topo))

    # Gráfico (use_container_width garante o ajuste no celular)
    ...
    st.info("Nenhum registro de queixa encontrado.")
